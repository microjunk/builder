...
public class WorldGenIndian extends StructureGeneratorBase
{
	...
	public WorldGenIndian(Entity entity, int[][][][] blocks)
	{
		super(entity, blocks);
	}

	public WorldGenIndian(Entity entity, int[][][][] blocks, int structureFacing) 
	{
		super(entity, blocks, structureFacing);
	}

	public WorldGenIndian(Entity entity, int[][][][] blocks, int structureFacing, int offX, int offY, int offZ) 
	{
		super(entity, blocks, structureFacing, offX, offY, offZ);
	}

	public WorldGenIndian() 
	{
		super();
	}
	...
	*/
	...
	public int getRealBlockID(int fakeID, int customData1) 
	{
		...
		switch(fakeID) 
		{
		case BigHut1.SPAWN_VILLAGER:
			return Block.torchWood.blockID;
		case Corral.SPAWN_HORSE1:
			return Block.torchWood.blockID;
		case Corral.SPAWN_HORSE2:
			return Block.torchWood.blockID;
		case Corral.SPAWN_HORSE3:
			return Block.torchWood.blockID;
		case Corral.SPAWN_HORSE4:
			return Block.torchWood.blockID;
		default:
	...
	*/
	...
	public void onCustomBlockAdded(World world, int x, int y, int z, int fakeID, int customData1, int customData2)
	{
		...
		case BigHut1.SPAWN_VILLAGER:
			...
			break;
		case Corral.SPAWN_HORSE1:
			Entity horse1 = new EntityHorse(world);
			((EntityHorse) horse1).setHorseType(0);
			((EntityHorse) horse1).setHorseVariant((int)Math.pow(2, (float)(8 + world.rand.nextInt(4))) + world.rand.nextInt(7));
			// Now use the preset method to avoid spawning in walls
			spawnEntityInStructure(world, horse1, x, y, z);
			break;
		case Corral.SPAWN_HORSE2:
			Entity horse2 = new EntityHorse(world);
			((EntityHorse) horse2).setHorseType(0);
			((EntityHorse) horse2).setHorseVariant(2);
			// Now use the preset method to avoid spawning in walls
			spawnEntityInStructure(world, horse2, x, y, z);
			break;
		case Corral.SPAWN_HORSE3:
			Entity horse3 = new EntityHorse(world);
			((EntityHorse) horse3).setHorseType(0);
			((EntityHorse) horse3).setHorseVariant(258);
			// Now use the preset method to avoid spawning in walls
			spawnEntityInStructure(world, horse3, x, y, z);
			break;
		default:
	...
	*/
	static
	{
		...
		structure.addBlockArray(StructureArrays.blockArrayBlackSmith);
